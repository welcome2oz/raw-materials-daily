...
매일 04:00(KST) Cowork 예약 작업이 `pipeline/RUNBOOK.md`의 P 절차로 기사를 수집해 발행함에 올리고(코드 실행 없음), 05:00 Claude Code 루틴이 받아 카드를 만들고 검증·push 하면, GitHub Actions가 **07:00에 인스타그램에 자동 게시**한다(저장소 루트 `README.md`). 결과와 기록은 저장소 `issues/<날짜>/`·`published/<날짜>.json` 에 쌓인다.
...
│  ├─ pickup.py            [A] 발행함의 Cowork 수집 자료(feeds/<날짜>/)를 runs/<날짜>/ 로 배치 (--feed)
│  ├─ hub_stage.py         (예전 방식, 쓰지 않음) 카드까지 만든 호를 발행함에 올리던 스크립트
